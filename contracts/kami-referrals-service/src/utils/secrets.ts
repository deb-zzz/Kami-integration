...
async function getEncryptionKey(): Promise<Buffer> {
	...
	if (!encryptionKey) {
		throw new Error(
			'ENCRYPTION_KEY environment variable is required. Generate a 32-byte hex string (64 hex characters).'
		);
	}
...
function getPrivateKeyFromEnv(): string | null {
	...
	if (process.env.NODE_ENV !== 'test') {
		console.warn(
			'⚠️  Using PRIVATE_KEY environment variable is deprecated. Use quest_pk database table instead.'
		);
	}
...
async function getPrivateKeyFromDatabase(chainId: number | string): Promise<string | null> {
	try {
		...
		const record = await prisma.quest_pk.findUnique({
			where: { chainId: chainIdHex },
			select: { pk: true },
		});

		return record?.pk || null;
	} catch (error) {
		// Database error - log but don't throw, allow fallback
		if (process.env.NODE_ENV !== 'test') {
			console.error(`Database error retrieving private key for chain ${chainId}:`, error instanceof Error ? error.message : 'Unknown error');
		}
		return null;
	}
}
 ...
 */
export async function getPrivateKeyByChainId(chainId: number | string): Promise<`0x${string}`> {
	const chainIdHex = chainIdToHex(chainId);

	// Try database first (primary source)
	let encryptedKey = await getPrivateKeyFromDatabase(chainId);
...
export async function storePrivateKey(chainId: number | string, encryptedKey: string): Promise<void> {
	const chainIdHex = chainIdToHex(chainId);
	await prisma.quest_pk.upsert({
		where: { chainId: chainIdHex },
		update: { pk: encryptedKey },
		create: {
			chainId: chainIdHex,
			pk: encryptedKey,
		},
