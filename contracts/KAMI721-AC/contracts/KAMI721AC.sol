...
contract KAMI721AC is AccessControl, ERC721Enumerable, ERC2981, Pausable {
    ...
    function sellToken(address to, uint256 tokenId, uint256 salePrice) external {
        ...
        KamiTransfer.sellToken(paymentToken, tokenId, to, salePrice, seller);
    ...
    function rentToken(uint256 tokenId, uint256 duration, uint256 rentalPrice) external whenNotPaused {
        ...
        KamiRental.rentToken(paymentToken, tokenId, duration, rentalPrice, tokenOwner, address(this));
    ...
    function extendRental(uint256 tokenId, uint256 additionalDuration, uint256 additionalPayment) external whenNotPaused {
        address tokenOwner = ownerOf(tokenId);
        KamiRental.extendRental(paymentToken, tokenId, additionalDuration, additionalPayment, tokenOwner);
    }
    ...
    function payTransferRoyalty(address to, uint256 tokenId, uint256 salePrice) external {
        KamiTransfer.payTransferRoyalty(paymentToken, tokenId, salePrice, ownerOf(tokenId), msg.sender);
    }
