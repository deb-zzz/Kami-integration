...
library KamiRoyalty {
    ...
    /**
     * @dev Collect payment from payer to contract, deduct platform commission, optionally set token mint royalties, and distribute mint royalties.
     * Used by mint/claim/batch flows to avoid duplicated bytecode.
     */
    function processMintPayment(
        ...
        if (amount > 0) {
            paymentToken.safeTransferFrom(payer, contractAddress, amount);
            uint96 platformCommission = KamiPlatform.platformCommission();
            uint256 commissionAmount = 0;
            if (platformCommission > 0) {
                commissionAmount = (amount * platformCommission) / 10000;
                if (commissionAmount > 0) {
                    paymentToken.safeTransfer(KamiPlatform.platformAddress(), commissionAmount);
                }
            }
            uint256 remainingAmount = amount - commissionAmount;
            if (mintRoyalties.length > 0) {
                setTokenMintRoyalties(tokenId, mintRoyalties, exists);
            }
            distributeMintRoyalties(tokenId, remainingAmount, paymentToken);
        }
    }

    /**
     * @dev Process mint payment when tokens are already in the contract (e.g. batchClaimFor).
     * Deducts platform commission, optionally sets token mint royalties, distributes mint royalties.
     * @param paymentToken ERC20 payment token
     * @param amount Amount already held by the contract to process
     * @param tokenId Token ID for royalty configuration
     * @param mintRoyalties Optional token-specific mint royalties
     * @param exists Callback to check token existence
     */
    function processMintPaymentFromReserve(
        IERC20 paymentToken,
        uint256 amount,
        uint256 tokenId,
        KamiNFTCore.RoyaltyData[] calldata mintRoyalties,
