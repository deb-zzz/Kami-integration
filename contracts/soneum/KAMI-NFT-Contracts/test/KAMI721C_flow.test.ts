...
describe('KAMI721C Royalty and Commission Distribution Flow', function () {
	...
	describe('Mint Flow - Royalty and Commission Distribution', function () {
		beforeEach(async function () {
			...
			await kami721c.setRoyaltyPercentage(ROYALTY_PERCENTAGE);
			...
			await kami721c.setMintRoyalties(mintRoyalties);
		});
		...
		it('Should distribute funds correctly on mint', async function () {
			...
			const initialContractBalance = await paymentToken.balanceOf(await kami721c.getAddress());

			// Mint token with price 100
			await kami721c.connect(user1).mint(user1.address, MINT_PRICE, 'https://example.com/token/1', []);
			...
			const finalContractBalance = await paymentToken.balanceOf(await kami721c.getAddress());
	...
	describe('Sale Flow - Royalty and Commission Distribution', function () {
		beforeEach(async function () {
			...
			await kami721c.setTransferRoyalties(transferRoyalties);
