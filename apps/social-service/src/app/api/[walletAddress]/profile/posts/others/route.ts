/**
 * @fileoverview API route handler for retrieving posts liked or shared by a user.
 *
 * This endpoint returns posts that have been either liked or shared by the specified wallet address.
 * Posts are ordered by creation date (oldest first) and include repost counts.
 *
 * @route GET /api/[walletAddress]/profile/posts/others
 * @module api/[walletAddress]/profile/posts/others
 */
...
/**
 * Handles GET requests to retrieve posts liked or shared by a specific wallet address.
 *
 * Returns posts where the user has either:
 * - Liked the post, OR
 * - Shared the post
 *
 ...
 *
 * @example
 * GET /api/0x123.../profile/posts/others
...
export async function GET(req: NextRequest, { params }: { params: { walletAddress: string } }) {
	...
	const posts = await prisma.post.findMany({
		where: {
			AND: [
				{
					// Posts that have been liked or shared by the wallet address
					OR: [
						{
							likes: {
								some: {
